...
function CompanionFormPage() {
	...
	const onDelete = async () => {
		await deleteCompanion.mutateAsync({ companionId, apiPath: ApiPaths.DiscordConfigs });
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		try {
			if (isNewCompanion) {
				const response = await createCompanion.mutateAsync({ values, apiPath: ApiPaths.DiscordConfigs });
				router.push(`/${RoutePaths.CompanionsDiscord}/${response.data._id}`);
			} else {
				await updateCompanion.mutateAsync({ companionId, values, apiPath: ApiPaths.DiscordConfigs });
			}
