...
import { useFetchWhatsAppConfig } from '@/queries/companions';
...
import OnlyContactsField from './OnlyContactsField';
...
const formSchema = z
	.object({
		...
		contactsWhitelist: z.array(z.string()),
...
function CompanionFormPage() {
	...
	const { data: config, refetch } = useFetchWhatsAppConfig(companionId);
	...
	const onlyContacts = form.watch('onlyContacts');
	const contactsFilterType = form.watch('contactsFilterType');
	...
	const onDelete = async () => {
		await deleteCompanion.mutateAsync({ companionId, apiPath: ApiPaths.WhatsappConfigs });
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		try {
			if (isNewCompanion) {
				const response = await createCompanion.mutateAsync({ values, apiPath: ApiPaths.WhatsappConfigs });
				router.push(`/${RoutePaths.CompanionsWhatsApp}/${response.data._id}`);
			} else {
				await updateCompanion.mutateAsync({ companionId, values, apiPath: ApiPaths.WhatsappConfigs });
			}
					...
					<Tabs defaultValue="general">
						...
						<TabsContent value="general">
							<div className="space-y-8 mt-5">
								...
								/>
								{companionId !== 'create' && enabled && (
									<DeviceLinkField config={config} refetchConfig={refetch} />
								)}
								<OnlyContactsField
									register={form.register}
									control={form.control}
									onlyContacts={onlyContacts}
									contactsFilterType={contactsFilterType}
								/>
							</div>
						</TabsContent>
