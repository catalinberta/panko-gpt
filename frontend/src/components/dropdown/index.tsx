...
function Dropdown<T extends FieldValues>(props: DropdownProps<T>) {
	...
	return (
		<div className="col-span-full">
			<label htmlFor="projectId" className="block text-sm font-medium leading-6 text-gray-300">
				{label}
			</label>
			<select
				id="projectId"
			...
			{error && <div className="mt-1 text-red-500 text-xs">{error.message}</div>}
			{props.hint && <p className="mt-1 text-sm leading-6 text-gray-400">{props.hint}</p>}
		</div>
	);
}
