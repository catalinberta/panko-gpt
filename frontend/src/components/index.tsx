...
export default function Breadcrumbs() {
	...
	const crumbs = () => {
		const breadcrumbPaths = location.pathname.split('/').filter(crumb => crumb !== '');
		const crumbElements = breadcrumbPaths.map((crumb, index) => {
			...
			return (
				<div key={index + 1} className="flex">
					...
					{index !== breadcrumbPaths.length - 1 && <p className="mr-2 ml-2 select-none">/</p>}
				</div>
			);
		});
		...
		crumbElements.unshift(
			<div key={0} className="flex">
				...
				{crumbElements.length ? <p className="mr-2 ml-2 select-none">/</p> : <></>}
			</div>
		);
